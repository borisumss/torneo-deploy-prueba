...
def login(request):
    if request.method == 'GET':
        ...
        if user is not None:
            rez = delegado_Inscripcion.objects.filter(
                id_delegadoIns_id=user.pk)
            pre = delegado_PreInscripcion.objects.filter(
                id_delegadoPreIns_id=user.pk)
            if len(rez) > 0:
                if rez[0].estado_delegado_inscripcion == 'BAJA':
                    messages.warning(
                        request, "Este usuario ha sido dado de baja")
                    return render(request, 'login.html')
                else:
    ...
    if request.method == 'GET':
        ...
        if not request.user.is_anonymous:
            if not request.user.email.endswith('@admin.com'):
                ...
                inscripcion = Inscripcion(qr_inscripcion=qr2, fecha_inicio=fecha_inicio_ins, fecha_fin=fecha_fin_ins, monto_inscripcion=monto_inscripcion_ins, id_torneo=torneo)
                inscripcion.save()
...
def enviarSolicitud(request, id):
    ...
    inscritoPre = delegado_PreInscripcion.objects.filter(
        ci_delegado_Preinscripcion=request.POST.get('ci_delegado'))
    inscritoRez = delegado_Inscripcion.objects.filter(
    ...
    if len(inscritoPre) > 0 or len(inscritoRez) > 0:
        messages.error(
            request, "Ya se encuentra un usuario registrado con estos datos")
        return redirect('home')
                ...
                if tipo == 'REZAGADOS':
                    ...
                    if len(ins) == 0:
                        ...
                        if aux == 'PENDIENTE':
                            ...
                            messages.success(
                                request, "La solicitud no se encuentra disponible")
                            return redirect('solicitudes')
...
def aceptar(request, tipo, id):
    if request.method == 'GET':
        ...
        if tipo == 'REZAGADOS':
            solicitud = delegado_Inscripcion.objects.filter(id=id)
            solicitud.update(
                estado_delegado_inscripcion='ACEPTADO', id_delegadoIns_id=user.pk)
            id_delegado = user.pk
            ...
            delegado = Delegado(id_delegado=id_delegado, nombre_delegado=nombre_delegado,
                                ci_delegado=ci_delegado, telefono_delegado=telefono_delegado)
            delegado.save()
            ...
            equipo = Equipo(estado_inscripcion_equipo=estado_inscrip, id_delegado=id_deleg,
                            id_torneo=id_torneo, escudo_equipo=escudo, portada_equipo=portada)
            equipo.save()

        elif tipo == 'PREINSCRIPCION':
            solicitud = delegado_PreInscripcion.objects.filter(id=id)
            solicitud.update(
                estado_delegado_Preinscripcion='ACEPTADO', id_delegadoPreIns_id=user.pk)
            id_delegado = user.pk
            ...
            equipo = Equipo(estado_inscripcion_equipo=estado_inscrip,
                            id_delegado=id_deleg, id_torneo=id_torneo)
            equipo.save()
...
def verTorneo(request, id):
    ...
    if request.method == 'GET':
        ...
        equipos = Equipo.objects.filter(
            estado_inscripcion_equipo="INSCRITO", id_torneo=id)
    ...
    if request.method == 'GET':
        if not request.user.is_anonymous:
            if not request.user.email.endswith('@admin.com'):
                ...
                delPre = delegado_PreInscripcion.objects.filter(
                    estado_delegado_Preinscripcion='ACEPTADO')
                delRez = delegado_Inscripcion.objects.filter(
                    estado_delegado_inscripcion='ACEPTADO')
                preBaja = delegado_PreInscripcion.objects.filter(
                    estado_delegado_Preinscripcion='BAJA')
                rezBaja = delegado_Inscripcion.objects.filter(
                    estado_delegado_inscripcion='BAJA')
        ...
        if not request.user.is_anonymous:
            if not request.user.email.endswith('@admin.com'):
                ...
                if (tipo == 'PREINSCRIPCION'):
                    ...
                    messages.success(
                        request, "Delegado dado de baja correctamente")
        ...
        if not request.user.is_anonymous:
            if not request.user.email.endswith('@admin.com'):
                ...
                if (tipo == 'PREINSCRIPCION'):
                    ...
                    messages.success(
                        request, "Delegado dado de alta correctamente")
                    return redirect('delegados')
                elif tipo == 'REZAGADOS':
                    delegado = delegado_Inscripcion.objects.filter(id=id)
...
#                 return redirect('login')
#             else:
#                 torneo_actual = get_object_or_404(
#                     Torneo, torneo_estado=1)  # peligroso
#                 categorias = Categorias_Torneo.objects.filter(
#                     id_torneo=torneo_actual.pk)
#                 equipos = Equipo.objects.filter(
#                     estado_inscripcion_equipo="INSCRITO", id_torneo=torneo_actual.pk)
        ...
        if not request.user.is_anonymous:
            if not request.user.email.endswith('@delegacion.com'):
                ...
                if equipo.estado_inscripcion_equipo == 'PENDIENTE':
                    messages.error(
                        request, "Inscriba a su equipo primeramente")
                    return redirect('delegacionEquipo')
                else:
                    jugadores = Jugador.objects.filter(id_equipo=equipo.pk)
                    entrenador = Entrenador.objects.filter(
                        id=equipo.id_entrenador_equipo.pk)
                    delegado = Delegado.objects.get(
                        id_delegado=equipo.id_delegado.pk)
    ...
    if request.method == 'GET':
        if not request.user.is_anonymous:
            if not request.user.email.endswith('@delegacion.com'):
                ...
                cate = Categorias_Torneo.objects.filter(
                    id_torneo=id_equipo.id_torneo)
                preinscripcion = delegado_PreInscripcion.objects.filter(
                    id_delegadoPreIns=request.user)
                inscripcion = delegado_Inscripcion.objects.filter(
                    id_delegadoIns=request.user)
                print(preinscripcion)
                print(inscripcion)
                return render(request, 'inscribirEquipo.html', {
                    'equipo': id_equipo,
                    'pre': preinscripcion.first(),
                    'ins': inscripcion.first(),
                    'categorias': cate,
                    'equiposTodos': aux
                })
        else:
        ...
        if form.is_valid():
            ...
            equipo = Equipo.objects.filter(id_delegado=request.user.id)
            equipo.update(estado_inscripcion_equipo='PRE-INSCRITO')
            messages.success(request, "Información registrada correctamente")
    ...
    if request.method == 'GET':
        if not request.user.is_anonymous:
            if not request.user.email.endswith('@delegacion.com'):
                ...
                aux = ''
                if id_equipo.id_entrenador_equipo:
                    aux = id_equipo.id_delegado.ci_delegado + "*" + \
                        id_equipo.id_entrenador_equipo.ci_entrenador + "*"
                else:
                    aux = id_equipo.id_delegado.ci_delegado + "*"
                aux2 = ''
                for i in jugadores:
                ...
                categoria = Categorias_Torneo.objects.filter(
                    nombre_categoria=id_equipo.categoria_equipo, id_torneo=id_equipo.id_torneo)
...
def inscribirEntrenador(request, id):
    if request.method == 'GET':
        ...
        entrenador = Entrenador(ci_entrenador=ci, nacimiento_entrenador=fecha, telefono_entrenador=telefono,
                                foto_entrenador=foto, nombre_entrenador=nombre, apodo_entrenador=apodo, nacionalidad_entrenador=nacionaldiad)
        entrenador.save()
    ...
    if request.method == 'GET':
        if not request.user.is_anonymous:
            if request.user.email.endswith('@delegacion.com'):
                ...
                return render(request, 'editarTorneo.html', {'torneo': torneo[0], 'preIns': preIns[0], 'rez': rez[0]})
        else:
            return redirect('login')
    elif request.method == 'POST':
        print(request.POST)
    ...
    if request.method == 'GET':
        if not request.user.is_anonymous:
            if not request.user.email.endswith('@admin.com'):
                ...
                equipoA = Equipo.objects.filter(
                    nombre_equipo=equipoA, id_torneo=id)
                equipoB = Equipo.objects.filter(
                    nombre_equipo=equipoB, id_torneo=id)
                ...
                nuevoA = sorted(jugadoresA, key=lambda x: int(
                    x.dorsal_jugador), reverse=False)
                nuevoB = sorted(jugadoresB, key=lambda x: int(
                    x.dorsal_jugador), reverse=False)
                return render(request, 'enfrentamiento.html', {
                    'equipoA': equipoA.first(), 'equipoB': equipoB.first(), 'id': id,
        ...
        equipoAO = Tabla_posiciones.objects.get(
            nombre_equipo=equipoA, id_torneo=id)
        equipoBO = Tabla_posiciones.objects.get(
            nombre_equipo=equipoB, id_torneo=id)
        ...
        if int(ptsequipoA) > int(ptsequipoB):
            ...
            print(equipoTB)
            equipoTA.update(partidos_jugados=partidosA, partidos_ganados=ganadosA, puntos_favor=favorA,
                            puntos_encontra=encontraA, puntaje_total=puntosA, diferencia=diferenciaA)
            equipoTB.update(partidos_jugados=partidosB, partidos_perdidos=perdidosB,
                            puntos_favor=favorB, puntos_encontra=encontraB, diferencia=diferenciaB)
            ...
            equipoTA = Tabla_posiciones.objects.filter(
                nombre_equipo=equipoA, id_torneo=id)
            equipoTB = Tabla_posiciones.objects.filter(
                nombre_equipo=equipoB, id_torneo=id)
            equipoTB.update(partidos_jugados=partidosB, partidos_ganados=ganadosB, puntos_favor=favorB,
                            puntos_encontra=encontraB, puntaje_total=puntosB, diferencia=diferenciaB)
            equipoTA.update(partidos_jugados=partidosA, partidos_perdidos=perdidosA,
                            puntos_favor=favorA, puntos_encontra=encontraA, diferencia=diferenciaA)
        ...
        for i in range(largo):
            ...
            estadisticas = Estadisticas_enfrentamiento(
                id_jugador=jug, id_enfrentamiento=enf, faltas=faltas, anotaciones=anotaciones)
            estadisticas.save()

            ptsJug = jug.anotaciones + int(anotaciones)
            faltasJug = jug.faltas + int(faltas)

            jugAux = Jugador.objects.filter(
                id=request.POST.getlist('IDJug')[i])
